...
def internal_links_job(self,url, maximum):
    ...
    div, script = generate_graph_internal_link_interactive(url, maximum)

    InternalLinks.objects.filter(task_id=self.request.id).update(div=div, script=script, status_job="FINISHED")
    return "Hello World!"
